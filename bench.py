...
def run_benchmarks(benchmarks, engines):
    ...
    subprocess.check_call(f"hyperfine --warmup 3 --runs 10 --export-json bench_results/wasmfx_results.json --export-csv bench_results/wasmfx_results.csv -L benchmark {','.join(benchmarks)} -L engine {','.join(engines)} 'run-scripts/./{{benchmark}}_{{engine}}_wasmfx.sh'", shell=True)
    # and now asyncify benchmarks
    subprocess.check_call(f"hyperfine --warmup 3 --runs 10 --export-json bench_results/asyncify_results.json --export-csv bench_results/asyncify_results.csv -L benchmark {','.join(benchmarks)} -L engine {','.join(engines)} 'run-scripts/./{{benchmark}}_{{engine}}_asyncify.sh'", shell=True)

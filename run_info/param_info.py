...
# We sed the pi and roundrobin for number of fibers
pi_tasks_param = subprocess.check_output(["awk", '/#define NUM_TASKS/ { sub(".* ", ""); print }', "examples/pi.c"], text=True).strip()
roundrobin_workers_param = subprocess.check_output(["awk", '/#define NUM_WORKERS/ { sub(".* ", ""); print }', "examples/roundrobin.c"], text=True).strip()
...
roundrobin_switches_param = subprocess.check_output(["awk", '/#define SWITCHES/ { sub(".* ", ""); print }', "examples/roundrobin.c"], text=True).strip()
    ...
    with open(f"{str(arg)}/param_info.json", "w") as f:
        json.dump({
            ...
            "pi_switch":
                ("tasks= " + pi_tasks_param + ", yields= " + pi_yields_param),
            "roundrobin":
                ("workers= " + roundrobin_workers_param + ", switches= " + roundrobin_switches_param),
            "roundrobin_switch":
                ("workers= " + roundrobin_workers_param + ", switches= " + roundrobin_switches_param),
            "c10m" : "N/A",
            "c10m_switch" : "N/A",

...
fiber_t fiber_sized_alloc(size_t stack_size, fiber_entry_point_t entry) {
  ...
  assert(fiber != NULL && "fiber_alloc failed: malloc returned NULL");
#if defined STACK_POOL_SIZE && STACK_POOL_SIZE > 0
  (void)stack_size;

...
__attribute__((noinline)) void *fiber_resume_with(fiber_t fiber, void *arg,
  ...
  // Note:
  //  1. the entry function must be run first both when the fiber is started and
  //  resumed!
  //  2. the prompt provided is only a placeholder and does change anything
  void *fiber_result = fiber->entry(0, arg);
